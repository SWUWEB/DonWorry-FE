...
const PURPOSES = [
  ...
  { id: 'car', emoji: '🚗', label: '자동차', description: '내 차 마련을 위해 저축해요.' },
  {
    ...
    emoji: '💰',
    label: '목돈 마련',
    description: '목돈을 만들어 재정적 안정을 높여요.',
  },
  {
    id: 'house',
    emoji: '🏠',
    label: '내 집 마련',
    description: '내 집 마련의 꿈을 향해 나아가요.',
  },
  {
    id: 'retire',
    emoji: '👴',
    label: '노후 준비',
    description: '편안한 노후를 위해 미리 준비해요.',
  },
  {
    id: 'selfdev',
    emoji: '📖',
    label: '자기계발',
    description: '나에게 투자하는 가장 가치 있는 소비예요.',
  },
      ...
      <div className={styles.content}>
        ...
        <div className={styles.labelRow}>
          <div className={styles.labelInner}>
            ...
            {selectedId && (
              <button className={styles.resetBtn} onClick={() => setSelectedId(null)}>
                초기화
              </button>
            )}
          </div>
        ...
        <div className={styles.amountSection}>
          <label htmlFor="amount-input" className={styles.amountLabel}>
            월 목표 금액
          </label>
          <div className={`${styles.inputWrap} ${amountError ? styles.inputWrapError : ''}`}>
            ...
            />
            <span className={`${styles.wonLabel} ${amountInput ? styles.wonLabelActive : ''}`}>
              원
            </span>
          </div>
          {amountError ? (
            <p id="amount-error" className={styles.errorMsg}>
              {amountError}
            </p>
          ) : isAmountValid ? (
        ...
        {selectedPurpose && !saveError && (
          <div className={styles.selectedTags}>
            <span className={styles.tag}>
              {selectedPurpose.emoji} {selectedPurpose.label}
            </span>
